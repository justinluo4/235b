...
def DrawCharacter(T_bt, character, T6t=None, z0=0.10, char_width=0.03, char_height=0.05):
    ...
    for (sx, sy), (ex, ey) in segments:
        ...
        commands.append({
            'type': 'translate',
            'target': [sx, sy, z0],
            'comment': f'Move above ({sx:.3f},{sy:.3f})'
        })
        # Lower pen to board (translate command to approach table)
        commands.append({
            'type': 'translate',
            'direction': [0, 0, -1],
            'distance': z0 - pen_z,
            'comment': 'Lower pen to board'
        })
        # Draw the segment (translate command to create the letter)
        dx, dy = ex - sx, ey - sy
